...
//	static String path = System.getProperty("user.dir");
	...
	public jxlFunctions() throws BiffException, IOException {
		Workbook inventoryInputSheet = Workbook
				.getWorkbook(new File(projPath+"TDM InputSheet_Final.xls"));
		// Inventory Sheet
		jxlFunctions.sh = inventoryInputSheet.getSheet("TDM_Inventory_Sheet");
		jxlFunctions.rowInv = sh.getRows();
		jxlFunctions.colInv = sh.getColumns();
		// SKU sheet
		jxlFunctions.shsku = inventoryInputSheet.getSheet("Total Products");
		jxlFunctions.rowSku = shsku.getRows();
		jxlFunctions.colSku = shsku.getColumns();
		// Store sheet
		jxlFunctions.shstore = inventoryInputSheet.getSheet("Store");
		jxlFunctions.rowstore = shstore.getRows();
		jxlFunctions.colstore = shstore.getColumns();
		System.out.println(rowstore);
		jxlFunctions.shbend = inventoryInputSheet.getSheet("Backend");
		jxlFunctions.rowbend = shbend.getRows();
		jxlFunctions.colbend = shbend.getColumns();
	}
	...
	public static ArrayList<String> getcolumnValuesFromIventorySheet(String columnName)
		...
		if (columnName == "ENVIRONMENT") {
			...
			column = 11;
		}else if (columnName == "LOCATION1") {
					column = 12;
		}else if (columnName == "LOCATION2") {
					column = 13;
		}else if (columnName == "LOCATION3") {
					column = 14;
		}
